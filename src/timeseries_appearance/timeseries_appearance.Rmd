...
metadata_file <- read_csv("../../raw_data/timeseries_dereplication/extended_metadata.csv", 
                          col_types = cols_only(
...
#keep first cdiff+ sample of each subject's timeseries
...
#age_ranges[[length(age_ranges)+ 1]] <- c(12, 24)
...
convert_to_ranges <- function(age_months, age_ranges){
  #min <- as.double(strsplit(substr(age_range, 1, nchar(age_range)-1), "_")[[1]][1])
  #max <- as.double(strsplit(substr(age_range, 1, nchar(age_range)-1), "_")[[1]][2])
#  age_months <- as.double(age_months)
  #way to avoid using for loop for all age ranges
  mapply(convert_range, age_months, MoreArgs=list(age_ranges=age_ranges))
  #ifelse(grepl("-", age_months, fixed=TRUE), strsplit(age_months, "-"),
  #       ifelse((age_months>min & age_months<max)|(age_months==min), age_range, "No"))
}
...
#this total is the universe of samples available for each age range: including cdiff+ and cdiff- timeseries
...
metadata_file_age_ranges.allLines_total_forPercentage.status <- metadata_file_age_ranges.allLines_total_forPercentage.status %>% 
  mutate(Other_variable = fct_relevel(Other_variable, "Total", "Healthy", "Diseased"),
         point_size = ifelse(total_forPercentage>=5 & total_forPercentage<50, 1, 
                      ifelse(total_forPercentage>=50 & total_forPercentage<100, 2, 
                             ifelse(total_forPercentage>=100 & total_forPercentage<500, 3, 
                                    # 0 for less than n=5 observations supporting the group
                                    ifelse(total_forPercentage>=500, 4, 0)))))
...
pdf(file="figures/first_appearance_cdiff_allLines.pdf", width=8, height=6)
